use rglk_game::actions::SelectedAction;
...
pub fn handle_tile_input(
    ...
    let active = item.get::<rglk_game::components::PlayerCharacter>().unwrap().active_card;
    let card_entity = actor.cards[active];
    let Some(card) = world.get_component::<rglk_game::components::Card>(card_entity) else { return };

    if let Some(cooldown) = world.get_component::<rglk_game::components::Cooldown>(card_entity) {
